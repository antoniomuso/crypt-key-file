...
module.exports = {
    ...
    createDecypheriv(algoritm, key_filename, iv) {
        return readFileKey(key_filename)
            .then((key) => {
                return Promise.resolve(crypto.createDecipheriv(algoritm, key, iv))
            })
    },

    createCypheriv(algoritm, key_filename, iv) {
        return readFileKey(key_filename)
            .then((key) => {
                return Promise.resolve(crypto.createCipheriv(algoritm, key, iv))
            })
    }
